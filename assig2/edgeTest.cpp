	...
	double * Gx, double * Gy, int X, int Y)
{
	...
	for (y = 2; y<(Y - 2); y++)
	{
		...
		done using the function greater_than() instead of the operators > or >= so numbers differing only
		due to rounding errors are considered equal */
		if (greater_than(mod, L) && !greater_than(R, mod) && gx >= gy) Dx = 1; /* H */
		else if (greater_than(mod, D) && !greater_than(U, mod) && gx <= gy) Dy = 1; /* V */
